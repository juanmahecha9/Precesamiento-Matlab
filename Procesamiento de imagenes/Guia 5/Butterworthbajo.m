function Ifil1 = Butterworthbajo(I,centro)
...
for x=1:M;
    for y=1:N;
        D(x,y)=(exp((-(sqrt((x-a)^2+(y-b)^2)))/(2*centro)));
    end
end
D=uint8(D);
L=graythresh(D);
...
Ifil1=uint8(Ifil);
% figure,
% subplot(1,2,1),imshow(Ifil),title('Filtered Image using Low Pass type Butterworth')
